...
export interface Painter {
    ...
    /**
     * Поперечный градиент из двух цветов: по краям один, посередине другой.
     *
     * Отдельно от общего градиента потому, что таких заливок в кадре сотня —
     * по одной на ноту, — и все они одной формы, разного только цвета. Держать
     * под каждую пару цветов своё описание значит заводить их тысячами.
     */
    fillRoundBand(
        x: number,
        y: number,
...
export interface Engine {
    ...
    /**
     * Сколько раз в секунду имеет смысл наполнять буфер свечения. Это не про
     * красоту, а про цену: там, где наполнение стоит дорого, свет можно
     * обновлять реже кадра — он размыт и инерционен. Там, где оно почти
     * бесплатно, экономить нечего, а отставший на три кадра ореол за быстрой
     * нотой видно сразу.
     */
    readonly glowHz: number;
