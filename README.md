...
| `?worker=0` | рисовать в окне, а не в рабочем потоке |
...
Сцену рисует не главный поток, а рабочий: холст отдаётся ему через
`transferControlToOffscreen`, и все слои живут там. Кадр на слабой машине
занимает двести миллисекунд, и всё это время нажатая клавиша ждала бы своей
очереди — вместе с панелью настроек, полосой файла и звуком.

В главном потоке остаётся двойник сцены: он принимает ввод, ведёт транспорт и
звук и держит настройки — с ним же работает панель. Всё, что он делает, мост
пересказывает рисующему: ноты, время файла, размер окна, изменённые настройки.
Обратно приходит только сводка о кадрах для счётчика и весть о сбойном слое.
...
Звук поэтому не зависит от картинки вовсе: живая нота попадает в сэмплер сразу,
не дожидаясь кадра. На замере в главном потоке не осталось ни одной долгой
задачи — там, где сцена рисовала сама, их было по шестьдесят миллисекунд подряд.

Если браузер не умеет отдавать холст (`transferControlToOffscreen`), сцена
рисует по-прежнему в окне — тот же код, другой холст. Тем же переключается и
замер: `?worker=0`.
