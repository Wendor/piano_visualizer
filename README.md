...
замирает: имя, прогресс с перемоткой мышью, время, полный список скоростей
(×0.25…×2), повтор и кнопки дорожек. Дорожки показываются, когда их больше
одной; выключенная дорожка не звучит и не рисуется — так удобно смотреть партию
одной руки.

Поддержаны форматы 0 и 1, running status, карта темпа и педаль (CC64). Тики
переводятся в секунды при разборе, поэтому дальше по проекту время везде в
секундах. Файлы с делением SMPTE отклоняются с сообщением в подсказке.
