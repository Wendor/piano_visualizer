...
Свечение живёт в кадровом буфере вчетверо меньше экрана; размытие — спуск по
пирамиде уменьшений и подъём обратно, теми же весами, что и на холсте: дороги
разные, картинка одна.

Сторона буфера округлена до кратной шестнадцати — до `2^(число ступеней)`.
Уменьшение вдвое мягкой выборкой даёт ровно среднее по четырём пикселям, но
только если сторона делится надвое без остатка. Не делится — и выборка едет:
одни пиксели усредняют пару, другие берут один, а картинка при этом движется.
Выходит биение: широкий ореол дышит с шагом в пиксель самой грубой ступени.
На восемнадцати кадрах в секунду этого не видно, на ста двадцати — видно
отчётливо, и выглядит оно как мерцание свечения.
...
и вспыхивает, то проходит между центрами и гаснет. В буфере свечения, который
вчетверо меньше экрана, тоньше пикселя почти всё: пылинка, искра, кант ноты.
